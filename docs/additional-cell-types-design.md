...
| Alternating Gate | Time topology: open and closed on a shared clock. | This GAMEPLAY-01 assessment rejected the underspecified decision-beat version. The later [Alternating Gate contract](alternating-gate.md) resolves phase, wait, sight, validation, multi-solver, and replay semantics. | The implemented form uses ordinary inventory/editing and identical fixed behavior. | Barred gate with explicit shape-based open/closed states. | **Historical decision superseded by the implemented shared one-second clock.** |
...
| Alternating Gate | Barred-gate icon; separated leaves when open and connected bars when closed | Cells: `n` / infinity; runtime state never changes the badge | `Alternating Gate n` / `Toggles every second.` |
