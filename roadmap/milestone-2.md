...
solver and normal-wall rules, so difficulty and progression can be evaluated before another solver or
...
- Normal walls and the existing deterministic random solver are the only gameplay mechanics in this
  milestone.

## Out of Scope

- A new solver behavior; Milestone 3 will define and introduce that mechanic.
- Multiple block types, limited inventories, or drag-and-drop placement.
- Multiple solvers or multiple start positions.
...
- Both levels remain playable with mouse or touch input in supported portrait and landscape viewports.
...
- [M2-09: Define Milestone 3's new solver type](done/milestone-2-09-define-milestone-3.md)

## Future Direction

Milestone 3 should add one meaningfully different solver behavior after the two-level progression is
...
larger change to game rules and input behavior and is intentionally deferred until the new solver can
